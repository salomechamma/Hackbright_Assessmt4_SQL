...
SELECT models.name as model, models.brand_name as brand, brands.headquarters as hq FROM models JOIN brands ON brands.name = models.brand_name WHERE brands.name = 'Ford' AND models.name='Mustang'; 
...
SELECT models.brand_name FROM models GROUP BY models.brand_name HAVING COUNT(models.name)> 5;
...
SELECT models.name, models.year FROM models JOIN brands on brands.name = models.brand_name WHERE models.year IN (SELECT brands.founded FROM brands);